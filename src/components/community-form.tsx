...
export function CommunityForm({ onCreated }: { onCreated?: () => void }) {
  ...
  async function submit() {
    ...
    onCreated?.();
    void navigate({ to: "/community/$id", params: { id: data.id as string } });
  }
    ...
    <div>
      <div className="space-y-5">
        ...
        <Field label="שם הקהילה *">
          <Input value={name} maxLength={80} placeholder="למשל: טניס נשים" onChange={(e) => setName(e.target.value)} />
        </Field>
        <Field label="תיאור">
          <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="מה עושים בקהילה?" />
        </Field>
        <div>
          <p className="mb-2 text-sm font-semibold">תחביב הקהילה *</p>
